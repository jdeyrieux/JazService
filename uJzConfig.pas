...
uses Windows, SysUtils, Consts, Classes, Contnrs, JvComponent, JvTrayIcon,
     ImgList, Controls, Menus, ExtCtrls, Graphics, BarMenus, System.StrUtils,
     uJzStruct, uJzCmnFunc, uJzSrvCtrl;
...
type
  ...
  TEnumIniSectionProc = procedure(const Line: string; const Ext: Longint) of object;
  ...
  protected
    ...
    procedure EnumAboutText(const Line: string; const Ext: Longint);
    procedure EnumConfig(const Line: string; const Ext: Longint);
    procedure EnumMenuItems(const Line: string; const Ext: Integer);
    procedure EnumMenuSettings(const Line: string; const Ext: Integer);
    procedure EnumMultiAction(const Line: string; const Ext: Integer);
    procedure EnumMessages(const Line: string; const Ext: Longint);
    procedure EnumServices(const Line: string; const Ext: Integer);
    procedure EnumVariables(const Line: string; const Ext: Integer);

    { Miscellaneous procs  }
    procedure BreakString(S: String; var Output: TBreakStringArray);
...
procedure TTMConfigReader.BreakString(S: String;
  var Output: TBreakStringArray);
var
  ParamName, Data: String;
  QuoteFound: Boolean;
  CurParm, CurPos, ColonPos, SemicolonPos, QuotePosBegin, QuotePosEnd: Integer;
begin
  CurParm := 0;
  CurPos := 1;
  while (Length(S) > CurPos) and (CurParm <= High(TBreakStringArray)) do begin
    ColonPos := Pos(':', S, CurPos);
    if ColonPos = 0 then
      ParamName := Copy(S, CurPos, Length(S) - CurPos - 1)
    else
      ParamName := Copy(S, CurPos, ColonPos - CurPos);
    ParamName := Trim(ParamName);

    if Length(ParamName) = 0 then
      AbortParsingFmt(SParsingParamHasNoValue, [S]);

    SemicolonPos := Pos(';', S, ColonPos);
    QuotePosBegin := Pos('"', S, ColonPos);
    QuotePosEnd := Pos('"', S, QuotePosBegin+1);
    QuoteFound := (QuotePosBegin > 0) and (QuotePosEnd > QuotePosBegin) and
                  (((SemicolonPos > 0) and (QuotePosEnd < SemicolonPos)) or (SemicolonPos = 0));

    if (QuotePosEnd < QuotePosBegin) then
...
//        AbortParsingFmt(SParsingParamMissingClosingQuote, [ParamName]);
    ...
    begin
      if (SemicolonPos > 0) then
        Data := Copy(S, ColonPos + 1, CurPos - ColonPos - 1)
      else
        Data := Copy(S, ColonPos + 1, CurPos - ColonPos)
    end
    else
      Data := Copy(S, QuotePosBegin + 1, QuotePosEnd - QuotePosBegin - 1);
    Data := Trim(Data);
...
begin
  ...
  for I := 0 to High(ParamInfo) do
    if CompareText(ParamInfo[I].Name, S.ParamName) = 0 then
    begin
      Result := I;
...
procedure TTMConfigReader.EnumAboutText(const Line: string;
  const Ext: Integer);
begin
  CustomAboutText.Add(Line);
end;

procedure TTMConfigReader.EnumConfig(const Line: string; const Ext: Longint);
    ...
    csImageList: begin
        try
          ...
          if PathIsAbsolute(Value) then
            ...
            LoadImageList(PathAppend(ExtractFileDir(Application.ExeName), Value));
        except
...
procedure TTMConfigReader.EnumMenuItems(const Line: string;
  const Ext: Longint);
const
        ...
        begin
          ...
          case CompareParamName(Params[P], ParamNames, ParamNameFound) of
            ...
            11: begin
                ...
                begin
                  ...
                  if TMAction is TTMServiceAction then
                    (TMAction as TTMServiceAction).Service := (Services[FoundIndex] as TTMService)
                  else
            ...
            15: begin
                ...
                with (TMAction as TTMHtmlWindowAction) do
                  while True do
                    case ExtractFlag(ParamData, uJzStruct.HtmlWindowFlags) of
                      -2: System.Break;
...
procedure TTMConfigReader.EnumMenuSettings(const Line: string; const Ext: Longint);
var
  KeyName, Value: String;
  I: Integer;
...
procedure TTMConfigReader.EnumMessages(const Line: string; const Ext: Longint);
var
  KeyName, Value: String;
  I: Integer;
...
procedure TTMConfigReader.EnumMultiAction(const Line: string;
  const Ext: Integer);
const
...
procedure TTMConfigReader.EnumServices(const Line: string;
  const Ext: Longint);
const
...
procedure TTMConfigReader.EnumVariables(const Line: string;
  const Ext: Longint);
