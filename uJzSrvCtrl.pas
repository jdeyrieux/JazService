...
type
  ...
  public
    constructor Create(const AContext: string; AErrorCode: Integer); overload;
  end;
...
constructor EServiceException.Create(const AContext: string; AErrorCode: Integer);
begin
  inherited Create(AContext + ' ' + SysErrorMessage(AErrorCode));
end;
...
begin
  ...
  if not CloseServiceHandle(ServiceHandle) then
    raise EServiceException.Create('Close service '+FServiceName, GetLastError);
  FServiceHandle := 0;

  if not CloseServiceHandle(SCManagerHandle) then
    raise EServiceException.Create('Close Servicecontrol '+FServiceName, GetLastError);
  FSCManagerHandle := 0;
end;
...
begin
  // lorsque le service n'est pas installer ne pas générer d'erreur
  // et ne pas permettre d'action sur le service
...
var
  ...
  Buffer: WinSvc.LPQUERY_SERVICE_CONFIG;
begin
  VerifyActive;
    ...
    begin
      ...
      try
        if not QueryServiceConfig(ServiceHandle, Buffer, BufferSizeNeeded,
          ...
          raise EServiceException.Create('GetServiceConfig '+FServiceName, GetLastError);
        Config := Buffer^;
      finally
        FreeMem(Buffer);
      end;  //try..finally
      ...
      raise EServiceException.Create('GetServiceConfig '+FServiceName, ErrorCode);
...
begin
  ...
  if not QueryServiceStatus(ServiceHandle, Status) then
    raise EServiceException.Create('GetServiceStatus '+FServiceName, GetLastError);
end;
...
begin
  ...
  if FSCManagerHandle = 0 then
    raise EServiceException.Create('Open ServiceControl '+FServiceName, GetLastError);

  // TODO :Chercher si le service existe à l'emplacement défini
...
begin
  ...
  if not StartService(ServiceHandle, 0, Parameters) then
    raise EServiceException.Create('Start service '+FServiceName, GetLastError);
end;
